...
float fBm(float3 p, int octaves, float lacunarity = 2.1, float gain = 0.5)
{
...
{
	float land0 = (float)0;
	float3 xSW = float3(-1, 0, -1);
	float3 xNW = float3(-1, 0, 1);
	float3 xNE = float3(1, 0, 1);
	...
	float2 uv = float2(id.xy) / float2(256, 256) + 0.5 / float2(256, 256);
	...
	//lerpedWorldPos = normalize(lerpedWorldPos);        // !!!!!???
	float d = fBm(lerpedWorldPos, 16);
	data[id.x] = d;
}